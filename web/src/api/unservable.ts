...
export function nameDaemons(ids: number[], racks: readonly { id: number; name: string }[]): string {
  // `rack N`, the same words `ScreensPage` and `TemplateCard` fall back to for
  // an unlisted rack. One template card used to say both -- "Attic on rack 63"
  // in the panel list and "daemon 63" in the notice directly above it -- which
  // reads as two machines when it is one.
  return ids.map((id) => racks.find((rack) => rack.id === id)?.name ?? `rack ${id}`).join(", ")
}
