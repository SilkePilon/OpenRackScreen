...
3. **The interface shows it** (§7) and an authenticated admin approves or
   denies.
4. **`GET /api/racks/claims/{id}`** — the daemon polls, authenticating every
   poll with an HMAC over the claim id under its identity secret. Without this,
   anyone who guesses or observes a claim id collects the key minted for
...
interface's approve dialog says so in those words.
...
- **Claim protocol**: the cap, the rate limit, expiry, deny-suppression, the
  HMAC check rejecting a poll with the wrong secret, and that the key is handed
  over exactly once and is absent from the second response.
